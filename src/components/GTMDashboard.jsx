...
function weekday(offset) {
  ...
  let added = 0;
  while (added < offset) { d.setDate(d.getDate()+1); if (d.getDay()!==0 && d.getDay()!==6) added++; }
  return d.toISOString().slice(0,10);
}
function buildVenueSchedule() {
  const schedule = {};
  VENUES.forEach((v,i) => { const dateStr = weekday(Math.floor(i/10)); if (!schedule[dateStr]) schedule[dateStr]=[]; schedule[dateStr].push(v); });
  return schedule;
}
...
const S = {
  ...
  navBtn: (active) => ({ padding:"12px 16px", fontSize:12, letterSpacing:"0.06em", textTransform:"uppercase", fontWeight:active?600:400, color:active?GOLD:CREAM+"99", background:"none", border:"none", borderBottom:active?`2px solid ${GOLD}`:"2px solid transparent", cursor:"pointer", transition:"all 0.15s" }),
  ...
  btn: (active, color=GOLD) => ({ padding:"6px 14px", fontSize:11, fontWeight:600, letterSpacing:"0.06em", textTransform:"uppercase", border:`1px solid ${active?color:LIGHT}`, borderRadius:4, cursor:"pointer", background:active?color:"transparent", color:active?"#fff":GREY, whiteSpace:"nowrap", transition:"all 0.15s" }),
  ...
  progressBar: () => ({ height:4, background:LIGHT, borderRadius:2, overflow:"hidden", marginTop:8 }),
  progressFill: (pct, color=GOLD) => ({ height:"100%", width:`${Math.min(pct,100)}%`, background:color, borderRadius:2, transition:"width 0.3s" }),
...
export default function App() {
  ...
  const set = useCallback((key, val) => { setState(prev => { const next={...prev,[key]:val}; saveState(next); return next; }); }, []);
  const get = useCallback((key, def="") => state[key] !== undefined ? state[key] : def, [state]);
...
function Today({ get, set, venueSent, coordEmailed, postsDone, todayStr }) {
  ...
  const todayCoords    = COORDINATORS.filter(c=>["to_email","followed"].includes(get(`c_status_${c.id}`,c.status))).slice(0,3);
    ...
    <div>
      <div style={S.section}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:20}}>
          {[{label:"Sessions Today",key:"metric_sessions_today"},{label:"Captures Today",key:"metric_captures_today"},{label:"Payments Today",key:"metric_payments_today"}].map(m=>(
            <div key={m.key} style={S.statBox}>
              <div style={S.statNum}>{get(m.key,"—")}</div>
              <div style={S.statLabel}>{m.label}</div>
              <input style={{...S.input,marginTop:8,textAlign:"center"}} placeholder="Enter number" value={get(m.key,"")} onChange={e=>set(m.key,e.target.value)} />
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={S.section}>
        ...
        {todayCoords.length===0
          ...
          : todayCoords.map(c=><CoordinatorCard key={c.id} c={c} get={get} set={set} compact />)
        }
      </div>
        ...
        {todayVenues.length===0
          ...
          : <div style={S.card}>
              <div style={{...S.row,marginBottom:12}}>
                ...
                <button style={S.btn(todayVenueSent===todayVenues.length)} onClick={()=>todayVenues.forEach(v=>set(`v_sent_${v.id}`,true))}>Mark All Done</button>
              </div>
              <div style={S.progressBar()}><div style={S.progressFill(todayVenueSent/todayVenues.length*100)} /></div>
            </div>
        }
      </div>
      ...
      <div style={S.section}>
        ...
        <div style={S.card}>
          {nextPost
            ? <><div style={{fontSize:12,color:GREY,marginBottom:4}}>Next post</div><div style={{fontWeight:600,fontSize:13}}>{nextPost.title}</div><div style={S.meta}>{nextPost.day} · {nextPost.type} · {weekTheme?.theme}</div></>
      ...
      <div style={S.section}>
        ...
        <textarea style={{...S.textarea,minHeight:80}} placeholder="Daily notes, follow-ups, ideas..." value={get(`notes_${todayStr}`,"")} onChange={e=>set(`notes_${todayStr}`,e.target.value)} />
      </div>
    </div>
  );
}

function CoordinatorCard({ c, get, set }) {
  ...
  function cycleStatus() { const idx=STATUS_CYCLE.indexOf(status); set(`c_status_${c.id}`,STATUS_CYCLE[(idx+1)%STATUS_CYCLE.length]); if(!date) set(`c_date_${c.id}`,today()); }
  return (
    <div style={{...S.card,borderLeft:`3px solid ${cfg.color}`}}>
      <div style={S.row}>
        ...
        <button style={S.btn(["emailed","coupon_sent","replied","activated"].includes(status),cfg.color)} onClick={cycleStatus}>{cfg.label}</button>
      </div>
      {open && (
        <div style={{marginTop:12}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
            <div><div style={{...S.meta,marginBottom:3}}>Date contacted</div><input type="date" style={S.input} value={date} onChange={e=>set(`c_date_${c.id}`,e.target.value)} /></div>
            <div><div style={{...S.meta,marginBottom:3}}>Status</div><select style={S.input} value={status} onChange={e=>set(`c_status_${c.id}`,e.target.value)}>{STATUS_CYCLE.map(s=><option key={s} value={s}>{C_STATUS[s].label}</option>)}</select></div>
          </div>
          <div style={{...S.meta,marginBottom:3}}>Notes</div>
          <textarea style={{...S.textarea,minHeight:60}} value={notes} onChange={e=>set(`c_notes_${c.id}`,e.target.value)} placeholder="Notes, replies, next steps..." />
        </div>
      )}
    </div>
  );
    ...
    <div>
      <div style={S.section}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:20}}>
          <div style={S.statBox}><div style={S.statNum}>{coordEmailed}</div><div style={S.statLabel}>Contacted</div><div style={S.progressBar()}><div style={S.progressFill(pct)} /></div></div>
          <div style={S.statBox}><div style={S.statNum}>{COORDINATORS.length-coordEmailed}</div><div style={S.statLabel}>Remaining</div></div>
      ...
      {filtered.map(c=><CoordinatorCard key={c.id} c={c} get={get} set={set} />)}
...
function Venues({ get, set, venueSent, todayStr }) {
  ...
  const searchResults = search.length>1 ? VENUES.filter(v=>v.name.toLowerCase().includes(search.toLowerCase())||v.email.toLowerCase().includes(search.toLowerCase())) : null;
  ...
  return (
    <div>
      <div style={S.section}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:20}}>
          <div style={S.statBox}><div style={S.statNum}>{venueSent}</div><div style={S.statLabel}>Sent</div><div style={S.progressBar()}><div style={S.progressFill(pct)} /></div></div>
          <div style={S.statBox}><div style={S.statNum}>{179-venueSent}</div><div style={S.statLabel}>Remaining</div></div>
          <div style={S.statBox}><div style={S.statNum}>179</div><div style={S.statLabel}>Total Venues</div></div>
        </div>
      </div>
      <div style={S.section}>
        <input style={{...S.input,marginBottom:16}} placeholder="Search venues by name or email..." value={search} onChange={e=>setSearch(e.target.value)} />
        {searchResults ? (
          <>{<div style={S.h3}>{searchResults.length} results for "{search}"</div>}{searchResults.map(v=><VenueRow key={v.id} v={v} get={get} set={set} />)}</>
        ) : (
          <>
            <div style={S.h2}>Daily Schedule — 10 venues/day</div>
            <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:16}}>
              {scheduleDays.map(d=>{
                const dv=VENUE_SCHEDULE[d]||[]; const ds=dv.filter(v=>get(`v_sent_${v.id}`)===true).length; const done=ds===dv.length; const isToday=d===todayStr;
                return <button key={d} style={{...S.btn(selectedDay===d||isToday),borderColor:done?GOLD:isToday?NAVY:LIGHT}} onClick={()=>setSelectedDay(d)}>{fmtDate(d)}{done&&<span style={{marginLeft:4,color:GOLD}}>✓</span>}</button>;
              })}
            </div>
            <div style={{...S.card,marginBottom:16}}>
              <div style={{...S.row,marginBottom:8}}>
                <div><div style={{fontWeight:600,fontSize:13}}>{fmtDate(selectedDay)} — {daySent}/{dayVenues.length} sent</div><div style={S.meta}>{dayVenues.slice(0,3).map(v=>v.name).join(", ")}{dayVenues.length>3?` +${dayVenues.length-3} more`:""}</div></div>
                <button style={S.btn(daySent===dayVenues.length&&dayVenues.length>0)} onClick={()=>dayVenues.forEach(v=>set(`v_sent_${v.id}`,true))}>Mark All Sent</button>
              </div>
              <div style={S.progressBar()}><div style={S.progressFill(dayVenues.length>0?daySent/dayVenues.length*100:0)} /></div>
            </div>
    ...
    <div style={{...S.card,borderLeft:`3px solid ${sent?GOLD:LIGHT}`}}>
      <div style={S.row}>
        ...
        <div style={{display:"flex",gap:6,flexDirection:"column",alignItems:"flex-end"}}>
          <button style={S.btn(sent)} onClick={()=>set(`v_sent_${v.id}`,!sent)}>{sent?"Sent ✓":"Mark Sent"}</button>
          {sent&&<button style={{...S.btn(replied,"#5BB87A"),fontSize:10}} onClick={()=>set(`v_replied_${v.id}`,!replied)}>{replied?"Replied ✓":"Replied?"}</button>}
        </div>
      </div>
      {open&&(
        <div style={{marginTop:12}}>
          <div style={{marginBottom:8}}><div style={{...S.meta,marginBottom:3}}>Date sent</div><input type="date" style={S.input} value={date} onChange={e=>set(`v_date_${v.id}`,e.target.value)} /></div>
    ...
    <div>
      <div style={S.section}>
        ...
        <div style={{...S.card,marginBottom:20}}>
          <div style={S.row}>
            <div><div style={{fontWeight:600,fontSize:13}}>{postsDone} / {total} posts published</div><div style={S.meta}>Mon / Wed / Fri · 9 Jun – 18 Jul 2026</div></div>
            <div style={{fontSize:11,color:GOLD,fontWeight:600}}>{Math.round(postsDone/total*100)}%</div>
          </div>
          <div style={{...S.progressBar(),marginTop:8}}><div style={S.progressFill(postsDone/total*100)} /></div>
        </div>
      </div>
          ...
          {w.posts.map(p=>{
            ...
            return (
              <div key={p.id} style={{...S.card,borderLeft:`3px solid ${done?GOLD:LIGHT}`}}>
                <div style={S.row}>
                  <div><div style={{fontWeight:600,fontSize:13,color:done?GREY:NAVY}}>{p.title}</div><div style={S.meta}>{p.day} · <span style={{color:GOLD+"99"}}>{p.type}</span></div></div>
                  <button style={S.btn(done)} onClick={()=>set(`post_done_${p.id}`,!done)}>{done?"Published ✓":"To do"}</button>
                </div>
              </div>
            );
          })}
        ...
        return (
          <div key={w} style={S.card}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
              {[{label:"Sessions Started",key:"sessions"},{label:"Email Captures",key:"captures"},{label:"Payments",key:"payments"}].map(m=>(
                <div key={m.key}><div style={{...S.meta,marginBottom:3}}>{m.label}</div><input style={S.input} type="number" min="0" placeholder="0" value={get(prefix+m.key,"")} onChange={e=>set(prefix+m.key,e.target.value)} /></div>
              ))}
            </div>
            <div style={{marginTop:8}}><div style={{...S.meta,marginBottom:3}}>Notes</div><textarea style={{...S.textarea,minHeight:44}} value={get(prefix+"notes","")} onChange={e=>set(prefix+"notes",e.target.value)} placeholder="What moved this week, what didn't..." /></div>
          </div>
        );
      })}
      <div style={S.section}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}>
          {["sessions","captures","payments"].map(k=>{
            ...
            return <div key={k} style={S.statBox}><div style={S.statNum}>{total}</div><div style={S.statLabel}>{k.charAt(0).toUpperCase()+k.slice(1)}</div></div>;
          })}
        </div>
      </div>
    </div>
