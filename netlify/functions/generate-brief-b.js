...
- LAST SONG GROUND TRUTH: If a last song is provided, reference it by name in the Last Song section. This is the definitive closing moment the couple has named — treat it as fixed. Never suggest an alternative or describe a mood without anchoring it to their named song. The operational instruction must name the song explicitly.
- PRE-DRINKS ACT CONSISTENCY: The coordinator brief describes how to brief the pre-drinks act — not what to book. Reference an act type only if it is clear from the couple's moment answers. For R30k–R60k budgets, treat pre-drinks as a single-act window (60–90 minutes). Do not split pre-drinks across two acts at this budget level.
- PRE-DRINKS DURATION: Pre-drinks runs 60–90 minutes on a standard SA wedding timeline. If the moment answers suggest an act whose standard set is under 60 minutes, include an instruction to confirm extended set availability or supplement with a playlist.
- ENTRANCE ACT CONSISTENCY: Apply the ENTRANCE LIVE ACT RULE. Check dinner_live_or_recorded and dinner_style in the moment answers — if dinner is recorded, playlist, curated, or DJ-only (any wording that does not explicitly confirm live musicians), the entrance must be PA. Do not brief a live entrance act when dinner is non-live.
- NO INVENTED CONTENT: Do not invent acts, performances, dances, or events the couple did not mention. If first dance answers contain no parent dances or additional dances, omit them. If no surprise performances are mentioned, do not suggest them. The coordinator brief reflects only what the couple confirmed in their answers — nothing assumed, nothing added.
...
- NO INVENTED EVENTS: Do not include father-daughter dances, mother-son dances, parent dances, surprise performances, or any event not explicitly confirmed in the couple's moment answers. If the firstdance_additional field is 'No' or is absent, omit all mention of additional dances entirely. This applies to every section of the brief.
- DANCING SECTION CONTAINS WIND-DOWN: Any wind-down instruction belongs inside the Dancing section. Never create a separate **WIND-DOWN** heading. If the couple has described a wind-down, include it as the closing sentence of the Dancing section.
- SONG DATA GROUND TRUTH: If the couple has named specific songs for a moment in their answers, reference those songs in the coordinator brief instruction for that moment. Never state the couple has named no songs when song data is present in the moment answers.
- MUSIC PLAN SOURCE OF TRUTH: The Music Plan is the definitive reference for every moment. If the Music Plan recommends a live act for a moment, the coordinator brief must instruct a live act for that moment. If the Music Plan recommends recorded or PA, the coordinator brief must instruct recorded or PA. Never contradict the Music Plan's act type decision in any moment section. This applies especially to: entrance (live vs PA), pre-drinks (live act vs playlist), and dinner (live vs recorded).
...
export const handler = async (event) => {
  ...
  const { limited } = await checkRateLimit(ip, 'generate-brief-b')
