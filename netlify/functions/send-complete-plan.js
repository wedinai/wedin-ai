...
export const handler = async (event) => {
  ...
  const { limited } = await checkRateLimit(ip, 'send-complete-plan')
  ...
  const { email, coupleName, coupleBrief, milRecommendations } = body
  ...
  const soundtrackHtml = coupleBrief
    .split('\n')
    .map(line => {
  ...
  // Section 2 — Key Moments (name + recommendation only, no why/cost/instruction)
  const moments = (milRecommendations?.moments || []).filter(m => m.name && m.name !== 'Your Wedding')
  const momentsHtml = moments.map(m => `
    <p style="margin:24px 0 4px;font-family:'DM Sans',sans-serif;font-size:11px;font-weight:600;color:#C4922A;letter-spacing:0.1em;text-transform:uppercase;">${m.name}</p>
    <p style="margin:0 0 8px;font-family:'DM Sans',sans-serif;font-size:14px;color:#1C2B3A;line-height:1.75;">${m.recommendation || ''}</p>
  `).join('')
...
<div class="ew">
  <div class="es">
    ...
    <div class="eb">
      <h1 class="eh2">Your complete music plan.</h1>
      <p class="sub">${displayName} — everything in one place.</p>
      <hr class="div1">
      <p style="margin:0 0 8px;font-family:'DM Sans',sans-serif;font-size:11px;font-weight:600;color:#C4922A;letter-spacing:0.1em;text-transform:uppercase;">Your Wedding Soundtrack</p>
      ${soundtrackHtml}
      <hr class="div1">
      <p style="margin:0 0 8px;font-family:'DM Sans',sans-serif;font-size:11px;font-weight:600;color:#C4922A;letter-spacing:0.1em;text-transform:uppercase;">Your Music Plan — Key Moments</p>
      ${momentsHtml}
      <hr class="div1">
      <p style="margin:0 0 8px;font-family:'DM Sans',sans-serif;font-size:11px;font-weight:600;color:#C4922A;letter-spacing:0.1em;text-transform:uppercase;">Your Other Outputs</p>
      <p style="margin:0 0 8px;font-family:'DM Sans',sans-serif;font-size:14px;color:#1C2B3A;line-height:1.75;">Your coordinator brief and budget guide are waiting in the app. Use the link below to return any time.</p>
    </div>
    <div class="ef">
      <a class="cb" href="${returnUrl}">Return to your plan &rarr;</a>
      <hr class="fd">
      <div class="fw">
  ...
  try {
    await resend.emails.send({
      ...
      to: email.trim(),
      subject: `Your wedin.ai music plan is ready — ${displayName}`,
      html,
    })
    return {
      statusCode: 200,
    ...
    console.error('send-complete-plan Resend error:', err)
