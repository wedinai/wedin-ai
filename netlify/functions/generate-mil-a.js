...
ENTRANCE LIVE ACT RULE — HARD PROHIBITION: If dinner_live_or_recorded contains 'playlist', 'curated', 'recorded', or 'background only', you MUST recommend PA for the entrance. You are prohibited from recommending any live act for the entrance in this case. Do not recommend a live entrance and then warn about it — simply recommend PA and explain that a live act for a 90-second entrance without a following live set is poor value. The only exception is if the same live act is explicitly continuing into dinner. The pre-drinks act ending and the entrance are two separate moments — a pre-drinks act that 'continues' into the entrance is still a live entrance act; if dinner is recorded, playlist, or DJ-only, the entrance MUST be PA regardless of whether a live act is already on-site from pre-drinks.
