...
export const handler = async (event) => {
  ...
  const { limited } = await checkRateLimit(ip, 'send-music-plan')
  ...
  const { email, content, coupleName } = body
...
<div class="ew">
  <div class="es">
    ...
    <div class="eb">
      <h1 class="eh2">Your music plan.</h1>
      <p class="sub">${coupleName && coupleName !== 'Your Wedding' ? coupleName : 'Your wedding'} — moment by moment.</p>
      <hr class="div1">
      ${contentHtml}
    </div>
    <div class="ef">
  ...
  try {
    ...
    console.error('send-music-plan Resend error:', err)
