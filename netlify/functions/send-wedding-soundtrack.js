...
export const handler = async (event) => {
  ...
  const { limited } = await checkRateLimit(ip, 'send-wedding-soundtrack')
  ...
  const { userEmail, coupleBrief, coupleName } = body
...
<style>
  ...
  .subnote { font-family: 'DM Sans', sans-serif; font-size: 13px; font-style: italic; color: #6B6560; text-align: center; margin: 0 0 28px; line-height: 1.5; }
...
<div class="ew">
  <div class="es">
    ...
    <div class="eb">
      <h1 class="eh2">Your wedding soundtrack.</h1>
      <p class="sub">${displayName} — every moment, the way you described it.</p>
      <hr class="div1">
      ${contentHtml}
    </div>
    <div class="ef">
      <a class="cb" href="${returnUrl}">Return to your Moment Map &rarr;</a>
      <p class="subnote">Your music plan, how to book, and your coordinator's brief are waiting when you're ready.</p>
      <hr class="fd">
      <div class="fw">
  ...
  try {
    await resend.emails.send({
      ...
      to: userEmail.trim(),
      subject: `Your wedding soundtrack — ${displayName}`,
      html,
    })
    return {
      statusCode: 200,
    ...
    console.error('send-wedding-soundtrack Resend error:', err)
