...
PRODUCTION CHECK COHERENCE: The productionCheck totalEstimate must account for all live acts across all nine moments — including moments 1–5 which are generated by a parallel function and not visible here. For moments 1–5, base estimates only on what the couple's own answers explicitly confirm — if a moment's answer indicates 'recorded', 'playlist', 'curated', or any non-live answer, do not include a live act cost for that moment in the total. Never produce a totalEstimate that reflects only the four moments in this batch. The total estimate must only include acts that were actually recommended across the nine moments. Never include an act category in the total that contradicts what the couple's answers indicate for that moment. Before stating a totalEstimate, sum the cost ranges stated in each moment's recommendation — use the lower bounds of each range to produce the minimum total and the upper bounds to produce the maximum total. State the total as a range: R[min]–R[max]. Do not generate a total estimate independently of the moment costs. If a moment uses recorded music or is included in a DJ booking at no extra cost, add R0 for that moment.
